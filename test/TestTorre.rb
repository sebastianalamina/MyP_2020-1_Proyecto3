...
class TestTorre < TestPieza
  ...
  def testDesplazar
    ...
    while i != 0
      ...
      while j != 0
        ...
        case i
        when 4
          TestMovimientos.testDerechaArriba(torre, k, 0, true)
        when 3
          TestMovimientos.testDerechaArriba(torre, 0, k, true)
        when 2
          TestMovimientos.testIzquierdaAbajo(torre, k, 0, true)
        when 1
          TestMovimientos.testIzquierdaAbajo(torre, 0, k, true)
        end
        j = j - 1
      end
    i = i - 1
  ...
  def testDesplazarMovimientosInvalidos
    ...
    while i != 0
      begin
        torre = Torre.new(rand(1..8), rand(1..8), true)
        TestMovimientos.testDerechaArriba(torre, rand(1.. 8), rand(1.. 8), false)
        assert false, mensaje_error + " de #{torre} [testDerechaArriba]"
      rescue ArgumentError => ae
      end
      begin
        torre = Torre.new(rand(1.. 8), rand(1.. 8), true)
        TestMovimientos.testDerechaAbajo(torre, rand(1.. 8), rand(1.. 8), false)
        assert false, mensaje_error + "#{torre} [testDerechaAbajo]"
      rescue ArgumentError => ae
      end
      begin
        torre = Torre.new(rand(1.. 8), rand(1.. 8), true)
        TestMovimientos.testIzquierdaArriba(torre, rand(1.. 8), rand(1.. 8), false)
        assert false, mensaje_error + "#{torre} [testIzquierdaArriba]"
      rescue ArgumentError => ae
      end
      begin
        torre = Torre.new(rand(1.. 8), rand(1.. 8), true)
        TestMovimientos.testIzquierdaAbajo(torre, rand(1.. 8), rand(1.. 8), false)
